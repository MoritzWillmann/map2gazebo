...
class MapConverter(object):
    ...
    def map_callback(self, map_msg):
        ...
        mesh_type = self.node.get_parameter_or("mesh_type", "stl").value
        #export_dir = rospy.get_param("~export_dir")
        export_dir = self.node.get_parameter("export_dir").value
        if mesh_type == "stl":
...
def main():
    ...
    node.declare_parameter("occupied_thresh")
    node.declare_parameter("box_height")
    node.declare_parameter("mesh_type")
    node.declare_parameter("export_dir")

    #map_topic = rospy.get_param("~map_topic", "map")
    map_topic = node.get_parameter_or("map_topic", "map")
    print(map_topic, map_topic.value)
    #occupied_thresh = rospy.get_param("~occupied_thresh", 1)
    occupied_thresh = node.get_parameter_or("occupied_thresh", 1).value
    #box_height = rospy.get_param("~box_height", 2.0)
    box_height = node.get_parameter_or("box_height", 2.0).value
